...
function buildServer() {
  ...
  server.tool(
    'get_current_signal',
    '获取美股当前进攻/防守信号。返回四档最终信号（attack进攻/neutral观望/reduce减仓观望/defense全面防守）、四个维度（AI供需/货币/财政/行政）各自的宽松/中性/收紧状态、以及全部参考指标（联邦基金利率、萨姆规则、联邦支出、WTI油价、EPU不确定性指数、美债收益率曲线、AI模型调用量等）。判定规则：进攻（非对称）=AI供需宽松且货币/财政/行政均不收紧（中性即可）且无锁且美债收益率曲线未处倒挂确认期（10y−3m近63个交易日中倒挂≥51天否决进攻准入）且信用利差未急剧走阔（Baa−10y利差90日走阔≥+60bp同样否决进攻准入）且实际利率未深度限制（政策利率−12M截尾PCE≥1.5%同样否决进攻准入）；仅单维收紧=减仓观望（语义：停止加仓提高警觉，非必须减存量仓位）；双维以上收紧或衰退锁激活=全面防守（例外：纯货币+财政共振降为减仓；上升趋势中树驱动与萨姆锁驱动的防守降级为减仓，应对式调整锁不受趋势否决；趋势地板：跌破10月SMA时最终档位至少减仓观望）。仅供研究参考，不构成投资建议。',
    {},
    async () => {
