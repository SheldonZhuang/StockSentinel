...
function buildFacts(payload) {
  ...
  return [
    ...
    `判定规则: 进攻(非对称)=AI供需宽松+货币/财政/行政均不收紧(中性即可)+无锁+美债收益率曲线(10y−3m)未处倒挂确认期+信用利差(Baa−10y)90日走阔<+60bp(任一否决器触发降为观望)；仅单维收紧=减仓观望；双维以上收紧或锁激活=全面防守(纯货币+财政共振只到减仓；上升趋势中树驱动与萨姆锁驱动的防守降级为减仓)`,
  ].join('\n');
}
