...
describe('VARIANTS_DEFAULT（基线口径守卫，2026-07-17 两轮 + 2026-07-18 第三轮评估定稿）', () => {
  it('V3(锁存2月)+V4(降档迟滞)+W5(趋势再入场)+X1(萨姆锁趋势门)+X3(纯货币财政降级) 默认开启，其余否决/搁置保持关闭', () => {
    expect(VARIANTS_DEFAULT).toEqual({
      ...
      trendReentry: true,
      // X系（2026-07-18 第三轮，准确率归因驱动）：X1+X3采纳；X1b砸08顶前入场、X2证明口径差
      // ≤0.1pp/年后否决、X4打穿08覆盖——lockTrendReentry/monetaryCarryDir/sahmConfirmMonths 保持缺省(关)
      sahmLockTrendReentry: true,
      defenseNeedsAdminOrLock: true,
      // M系（2026-07-18 第四轮，市场/估值第5维评估）：全部默认关，采纳由主会话定
      m1TrendVote: false,
      marketConfirmOnly: false,
      marketMonetaryReduce: false,
      m2DrawdownPct: null,
...
// ---------- M系（2026-07-18 第四轮：市场/估值第5维评估） ----------

describe('rollingHighDrawdownPct（M2：距52周最高收盘回撤）', () => {
  const bars = [
    { date: '2021-01-04', close: 100 },
    { date: '2021-06-01', close: 120 }, // 52周高点
    { date: '2021-12-30', close: 108 },
  ];
  it('最新收盘距窗口内最高收盘的回撤（%）', () => {
    expect(rollingHighDrawdownPct(bars, '2021-12-31', 365)).toBeCloseTo((108 / 120 - 1) * 100, 6);
  });
  it('窗口外的旧高点不参与（52周滚动）', () => {
    // asOf 2022-07-01：365天窗口起点 2021-07-01，120 高点已滚出 → 高点=108（最后一根自身）
    expect(rollingHighDrawdownPct(bars, '2022-07-01', 365)).toBeCloseTo(0, 6);
  });
  it('自身即高点 → 0', () => {
    expect(rollingHighDrawdownPct(bars, '2021-06-01', 365)).toBeCloseTo(0, 6);
  });
  it('窗口内无数据 → null', () => {
    expect(rollingHighDrawdownPct(bars, '2020-06-01', 365)).toBe(null);
    expect(rollingHighDrawdownPct([], '2021-12-31', 365)).toBe(null);
  });
});

describe('parseMultplCape（M3：multpl.com 月度表解析）', () => {
  const html = `
    <tr class="odd"> <td>Jul 17, 2026</td> <td> &#x2002; 41.52 </td> </tr>
    <tr class="even"> <td>Jul 1, 2026</td> <td> &#x2002; 41.30 </td> </tr>
    <tr class="odd"> <td>Jun 1, 2026</td> <td> &#x2002; 40.88 </td> </tr>
    <tr class="even"> <td>May 1, 1871</td> <td> &#x2002; 12.59 </td> </tr>`;
  it('解析日期与值，按月升序', () => {
    const out = parseMultplCape(html);
    expect(out.map(o => o.month)).toEqual(['1871-05', '2026-06', '2026-07']);
    expect(out[1].value).toBeCloseTo(40.88, 6);
  });
  it('同月多条（月中最新行+月初行）保留页面靠前的最新值', () => {
    const out = parseMultplCape(html);
    expect(out.find(o => o.month === '2026-07').value).toBeCloseTo(41.52, 6);
  });
  it('空/无法解析 → 空数组', () => {
    expect(parseMultplCape('<html></html>')).toEqual([]);
  });
});

describe('replayMonth M系变体（市场/估值票；默认全关时与基线逐位一致）', () => {
  const NEUTRAL_INPUT = { rate: 3, prevRate: 3, walcl: null, prevWalcl: null, fiscalChangePct: null, epuPercentile: null, sahm: null };
  const NO_LOCK = { sahmLockActive: false, reactiveLockActive: false };
  const MON_TIGHT = { ...NEUTRAL_INPUT, rate: 3.25, prevRate: 3 }; // +25bp：货币tight、不触发应对锁

  it('默认关：dd52wPct/capePercentile 存在也不影响档位', () => {
    const r = replayMonth({ ...MON_TIGHT, spxBelowSma10: true, dd52wPct: -20, capePercentile: 99 }, NO_LOCK);
    expect(r.final).toBe('reduce'); // 仅货币单维tight
  });

  it('M1：货币tight + 月末跌破10月SMA → 市场票凑成双维共振=defense', () => {
    const r = replayMonth({ ...MON_TIGHT, spxBelowSma10: true }, NO_LOCK, { m1TrendVote: true });
    expect(r.final).toBe('defense');
  });

  it('M1：高于SMA → 市场维neutral不投票（不影响进攻逻辑）', () => {
    const r = replayMonth({ ...MON_TIGHT, spxBelowSma10: false }, NO_LOCK, { m1TrendVote: true });
    expect(r.final).toBe('reduce');
  });

  it('M1：市场票单独tight（四维全不紧）→ 单票=减仓观望', () => {
    const r = replayMonth({ ...NEUTRAL_INPUT, spxBelowSma10: true }, NO_LOCK, { m1TrendVote: true });
    expect(r.final).toBe('reduce');
  });

  it('M1：货币+财政被X3降档时，市场票第3票恢复defense', () => {
    const X3 = { defenseNeedsAdminOrLock: true }; // 空变体=X3采纳前旧基线，需显式开启
    const noM = replayMonth({ ...MON_TIGHT, fiscalChangePct: 8, spxBelowSma10: true }, NO_LOCK, X3);
    expect(noM.final).toBe('reduce'); // X3：纯货币+财政 → reduce
    const r = replayMonth({ ...MON_TIGHT, fiscalChangePct: 8, spxBelowSma10: true }, NO_LOCK, { ...X3, m1TrendVote: true });
    expect(r.final).toBe('defense');
  });

  it('M1b 确认票：自己不凑数（货币tight+跌破SMA仍reduce）', () => {
    const r = replayMonth({ ...MON_TIGHT, spxBelowSma10: true }, NO_LOCK, { m1TrendVote: true, marketConfirmOnly: true });
    expect(r.final).toBe('reduce');
  });

  it('M1b 确认票：四维自身≥2共振被X3降档时 +1 票恢复defense', () => {
    const r = replayMonth({ ...MON_TIGHT, fiscalChangePct: 8, spxBelowSma10: true }, NO_LOCK,
      { defenseNeedsAdminOrLock: true, m1TrendVote: true, marketConfirmOnly: true });
    expect(r.final).toBe('defense');
  });

  it('M1x：恰"市场+货币"两票比照X3降级reduce；市场+行政不受影响', () => {
    const mm = replayMonth({ ...MON_TIGHT, spxBelowSma10: true }, NO_LOCK, { m1TrendVote: true, marketMonetaryReduce: true });
    expect(mm.final).toBe('reduce');
    const ma = replayMonth({ ...NEUTRAL_INPUT, epuPercentile: 92, spxBelowSma10: true }, NO_LOCK, { m1TrendVote: true, marketMonetaryReduce: true });
    expect(ma.final).toBe('defense');
  });

  it('M2：距52周高点回撤超阈值 → 市场票；阈值内不投', () => {
    const deep = replayMonth({ ...MON_TIGHT, dd52wPct: -12 }, NO_LOCK, { m2DrawdownPct: 10 });
    expect(deep.final).toBe('defense');
    const shallow = replayMonth({ ...MON_TIGHT, dd52wPct: -8 }, NO_LOCK, { m2DrawdownPct: 10 });
    expect(shallow.final).toBe('reduce');
    const th15 = replayMonth({ ...MON_TIGHT, dd52wPct: -12 }, NO_LOCK, { m2DrawdownPct: 15 });
    expect(th15.final).toBe('reduce');
  });

  it('M1+M2 同开 OR 合并为一票（不双计）：市场票+货币tight=2票defense，纯市场仍单票reduce', () => {
    const r = replayMonth({ ...NEUTRAL_INPUT, spxBelowSma10: true, dd52wPct: -20 }, NO_LOCK, { m1TrendVote: true, m2DrawdownPct: 10 });
    expect(r.final).toBe('reduce');
  });

  it('M3f 独立票：CAPE>90分位 + 货币tight → defense；≤90分位不投', () => {
    const hi = replayMonth({ ...MON_TIGHT, capePercentile: 95 }, NO_LOCK, { capeFullVote: true });
    expect(hi.final).toBe('defense');
    const lo = replayMonth({ ...MON_TIGHT, capePercentile: 85 }, NO_LOCK, { capeFullVote: true });
    expect(lo.final).toBe('reduce');
  });

  it('M3c 确认票：自己不凑数，仅恢复被X3降档的共振', () => {
    const alone = replayMonth({ ...MON_TIGHT, capePercentile: 95 }, NO_LOCK, { capeConfirmVote: true });
    expect(alone.final).toBe('reduce');
    const x3 = replayMonth({ ...MON_TIGHT, fiscalChangePct: 8, capePercentile: 95 }, NO_LOCK,
      { defenseNeedsAdminOrLock: true, capeConfirmVote: true });
    expect(x3.final).toBe('defense');
  });

  it('dd52w/cape 缺失(null) → fail-open 不投票', () => {
    const r = replayMonth({ ...MON_TIGHT, dd52wPct: null, capePercentile: null }, NO_LOCK, { m2DrawdownPct: 10, capeFullVote: true, capeConfirmVote: true });
    expect(r.final).toBe('reduce');
  });
});
