...
export const VARIANTS_DEFAULT = {
  ...
  // 效果（2026-07-18 采纳时点数值，此后 O1 油价水平护栏等采纳再有小幅刷新，当前口径以下文汇总为准）：全期年化 12.2→12.4%、纯误报防守段 3→2（假阳性严格口径 6/8→5/7）、防守月 79→76；
...
export function evaluate(D, timeline) {
  ...
  for (const ep of episodes) {
    ...
    if (!startPx) continue;
    const horizonAll = timeline.filter(t => t.month >= ep.start).slice(0, 13);
    const horizon = horizonAll.map(t => t.spx).filter(v => v !== null && v !== undefined && !isNaN(v));
    if (!horizon.length) continue;
    const minPx = Math.min(...horizon);
    const crashed = (minPx / startPx - 1) * 100 <= -15;
    // 末端截断护栏（2026-07-30 审查修复）：视界不足13个采样点且未见>15%回撤时，
    // 未来数据尚不足以证伪——"还没跌"不能记假阳性（危机进行中重跑会污染精确率）
    if (!crashed && horizonAll.length < 13) { fpUndecided++; continue; }
    if (!crashed) falsePositives++;
  }
  if (fpUndecided) {
    console.warn(`[backtest] ${fpUndecided} defense episode(s) too recent to judge (excluded from false-positive stats)`);
  }
...
> **"相对买入持有少亏"按实际曝险路径计算**：从首次防守信号月到危机底部月逐月复利——defense 月按现金（联邦基金利率月化，与全期模拟同口径），非 defense 月按 SPY 月收益；该值 = 路径收益 − 同期买入持有收益（百分点，正=少亏）。防守片段中途的临时解除窗口（如有）如实计入，**不再假设从首次信号一路防守到底部**（旧口径按 底部价/信号价−1 计算，系统性高估保护效果）。路径终点为底部月的月末采样价。
