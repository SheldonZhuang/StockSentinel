...
信号体系已过六轮专家复查与多轮方法论审查（判定逻辑 v2、防守分级、收益率曲线否决、趋势再入场、降档迟滞、capex 单季侦察兵规则均已实证定案），499 测试用例全过，每日云端自动运行并公开存档。

**后续迭代**：付费订阅墙的实际启用、移动端原生 App/小程序、Google OAuth 登录接入、capex 指引下修人工事件输入。
