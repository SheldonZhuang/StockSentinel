...
- **进攻（非对称）** = AI供需宽松（主动引擎发动）+ 货币/财政/行政均不收紧（中性即可，任一收紧即否决）+ 无锁 + 美债收益率曲线（10y−3m）未处倒挂确认期（连续倒挂 ≥63 个交易日否决进攻档准入）+ 信用利差（BAA10Y）90日走阔 <+60bp（急剧走阔否决进攻档准入，2026-07-30 采纳——不在信用恐慌中开最激进档位；只否决进攻，不触发防守）。
