...
server.tool(
  'get_current_signal',
  '获取美股当前进攻/防守信号。返回四档最终信号（attack进攻/neutral观望/reduce减仓观望/defense全面防守）、四个维度（AI供需/货币/财政/行政）各自的宽松/中性/收紧状态、以及全部参考指标（联邦基金利率、萨姆规则、联邦支出、WTI油价、EPU不确定性指数、收益率曲线、AI模型调用量等）。判定规则：进攻（非对称）=AI供需宽松且货币/财政/行政均不收紧（中性即可）且无锁且收益率曲线未处倒挂确认期（10y−3m连续倒挂≥3个月否决进攻准入）；仅单维收紧=减仓观望（语义：停止加仓提高警觉，非必须减存量仓位）；双维以上收紧或衰退锁激活=全面防守（例外：纯货币+财政共振降为减仓；上升趋势中树驱动与萨姆锁驱动的防守降级为减仓，应对式调整锁不受趋势否决）。仅供研究参考，不构成投资建议。',
  {},
