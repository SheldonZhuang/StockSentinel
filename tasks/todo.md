# 127b + 128号: 决议覆盖告警 + 事件型指标即时刷新 — 2026-09-17 【已完成】

## 127b 决议覆盖告警（用户："请补上"）
- [x] `utils/fed-coverage.js`：日历（该有决议）× Fed官网（是否真拿到）双源交叉判定
- [x] 三条"已落实"路径（任一成立即不告警）：①声明顶替生效 ②FRED台阶已收录 ③今天的声明已成功解析
      ——漏掉③会把每次"按兵不动"的决议误报为故障
- [x] 接入点：每日 cron（21:00）+ 14:05 事件扫描 + 每小时看门狗；同一决议日只发一次（发送失败撤回去重标记可重试）
- [x] 测试 14 项；实测：模拟"决议日无声明"发出 1 封、二次调用被去重

## 128 事件型指标即时刷新（用户原则："一经公布立即更新"）
- [x] `config/fomc-meetings.js` 新增 `isFomcDecisionDate` / `daysSinceLastFomcDecision`
- [x] `utils/event-refresh.js`：通用引擎，把 127 的决议专用实现抽为事件型通用路径
      - 白名单 `EVENT_REFRESHABLE_DIMENSIONS = ['monetary','fiscal']`（行政维依赖EPU/油价、AI供需依赖EDGAR/OpenRouter，均非事件型，绝不被本引擎改写）
      - `detectPublishedIndicators`：按**参考期前移**判定新发布（回退/缺失/旧库缺列都不触发）
      - 三条不变量与 127 一致：只UPDATE不INSERT / 降档守卫 / 失败降级为"等今晚21:00"
- [x] 调度：美东 08:35 / 10:05 / 14:05 三次定点扫描（对齐 BLS/BEA 08:30 发布节奏；14:05 兼 FOMC）
- [x] fed-refresh 改用通用引擎，消除两套重复判定链
- [x] 测试 19 项；实测：真实拉取下"本次检测到的新发布=无"（今日无新一期），空转正确

## 关键设计判断：不是所有指标都该实时化
用户原话是"所有参数一经公布立即更新"。落地时按发布节奏分成两类，**只对第一类做了实时化**：
- A 离散事件型（FOMC/08:30月度发布）：公布时刻固定、结果一经公布即永久确定 → 公布当天即时生效 ✅
- B 连续行情型（WTI油价/信用利差BAA10Y/收益率曲线T10Y3M/EPU日频）：时刻在变，而系统按"每日21:00采样点"
  记 track record。盘中实时化会让采样点从"固定时点"漂移成"最后一次访问的时刻" → **有意保持21:00**。
  日频指标滞后1-2天在既有判定里已被容忍（油价看30天窗口、曲线看63交易日窗口、信用利差看90日窗口）。
  这不是"没做"，是做了会破坏时间序列语义与回测口径（回测锚定收盘口径）。
- [ ] 若用户坚持要日频行情也实时化，需先决定：track record 采样点语义是否改为"每日最后一个观测"（会影响回测可比性）
...
- [x] 运维邮件通道本机实发 sent:1（replica 实例→ADMIN_EMAIL）；用户已确认收到 ✅
- [x] Cloudflare Radar 本机实测出数（28日趋势 −0.75%）；Railway Variables 已确认含 CLOUDFLARE_API_TOKEN ✅（两实例齐备）
...
- [x] 部署核验：Railway bc60c7fc Active（node 22.23.2/railpack 0.39，cron+日报+备份全跑通）；Vercel 首页/track-record/api 代理/llms.txt 全 200 ✅
