...
2. **Stripe Payment Links**（有真实付费意向时启用）：Stripe 后台建订阅链接 → 用户付款
   → 管理后台手动签发 pro key 并**绑定归属用户 + 设订阅到期时间**（123号，2026-08-15：
   到期后该用户的 pro key 自动按 free 配额计，key 不禁用，续费改到期时间即恢复——
   "剩余时间"有真实约束力，不再依赖人工记账）；量大后再做 webhook 自动化
3. **企业合同**：线下签，人工开 key
...
| 项 | 状态 |
|---|---|
| 限流计数持久化 + 按key用量底账 | ✅ 已上线（api_usage 表，10分钟批量落盘，保留400天） |
| 用户管理 + 按次调用明细监控 | ✅ 已上线（123号，2026-08-15：用户列表/订阅剩余/按用户端点统计/调用日志30天/全局功能热度，见 [admin-guide.md](admin-guide.md)） |
| 订阅到期自动降级 | ✅ 已上线（pro key 绑定用户订阅过期 → free 配额） |
