...
生成时间：2026-07-17 ｜ 数据源：FRED（利率 DFEDTAR+DFEDTARU 拼接、WALCL、MTSO133FMS+PCEPI平减(实际支出口径)、EPUTRADE、SAHMREALTIME）｜ 标普500：Tiingo SPY 总回报（adjClose，含股息复权）
...
| 2020 新冠崩盘 | 2020-02-19 | 2020-03-23 | -33.7% | 2019-10 | 提前111天 | 信号后再涨+12.1%见顶 | +14.8pp | 100.0% | 2020-05 | 决策树 |
| 2022 加息熊市 | 2022-01-03 | 2022-10-12 | -24.5% | 2022-05 | 滞后148天 | 已回落-13.3% | +6.6pp | 100.0% | 2023-03 | 应对式锁 |
| 2025 关税战 | 2025-02-19 | 2025-04-08 | -18.8% | 2024-12 | 提前50天 | 信号后再涨+4.6%见顶 | +6.6pp | 100.0% | 2025-10 | 决策树 |
...
- 全面防守期月均收益：**-0.3%**（120 个月）｜减仓观望期：**1.4%**（122 个月）｜其余：**1.4%**（75 个月）
- **全面防守占比：38%**，减仓观望占比：38%（防守分级后，单维收紧不再全仓防守）
- 全期（26.4年）：买入持有 年化 8.5%、最大回撤 -50.8%；策略（仅defense离场，defense月计现金利息）年化 11.7%、最大回撤 -16.2%
- **敏感性（照档位建议执行）**："reduce=50%仓"策略（reduce 月 50% SPY + 50% 现金利息；defense 月全现金；其余满仓）：年化 8.7%、最大回撤 -12.9%——与上行"仅defense离场"口径并列，反映严格按档位执行的真实预期
- **子样本稳健性（2010-01 起，16.4年）**：策略年化 12.3%（最大回撤 -16.2%）vs 买入持有年化 14.6%（最大回撤 -23.9%）——策略的超额收益高度依赖起点包含 2000 年泡沫顶；后半段策略主要贡献是降回撤而非增收益
- 各维度收紧月数：货币 40、财政 89、行政 85；锁激活 61 个月
- 防守信号片段共 **19** 段，其中 **17** 段未伴随随后12个月内 >15% 的回撤（假阳性率 89%）
...
6. **假阳性为保守上界**：假阳性判定用月末采样价，月中盘中出现过 >15% 但月末收复的回撤会被漏计，89% 是保守上界。
...
1. **捕获情况**：5/6 场危机触发防守信号，其中 3 场为提前捕获（信号早于顶部）、2 场为滞后捕获（相对顶部：滞后68天、提前11天、提前111天、滞后148天、提前50天、—）。滞后捕获多由应对式利率锁在危机演进中接管。
2. **防守分级已生效（2026-07-12 用户拍板）**：单维收紧=减仓观望（部分仓位），双维共振或锁=全面防守（空仓/对冲）。全面防守占比 38%（分级前为 74%），大量单维噪声月份降级为减仓观望。
3. **精确率仍是主要代价**：89% 的防守片段未跟随大回撤，防守期月均收益 -0.3%（为负，防守有效规避了下跌；vs 非防守 1.4%）。若严格按信号空仓执行会错过部分上涨月份。
