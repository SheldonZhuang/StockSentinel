...
生成时间：2026-09-07 ｜ 数据源：FRED（利率 DFEDTAR+DFEDTARU 拼接、WALCL、MTSO133FMS+PCEPI平减(实际支出口径)、EPUTRADE、SAHMREALTIME）｜ 标普500：Tiingo SPY 总回报（adjClose，含股息复权）

## 方法论

- **重放粒度**：月末采样，320 个月（2000-01 起），逐月用与线上完全一致的阈值（`signal.config.js`）重算四维信号、萨姆锁/应对式调整锁与最终信号。未收官的当前日历月（月中重跑时只有半个月数据）从重放中剔除。
...
- 全面防守期月均收益：**-0.8%**（76 个月）｜减仓观望期：**1.4%**（183 个月）｜其余：**0.9%**（60 个月）
- **全面防守占比：24%**，减仓观望占比：57%（防守分级后，单维收紧不再全仓防守）
- 全期（26.6年）：买入持有 年化 8.5%、最大回撤 -50.8%；策略（仅defense离场，defense月计现金利息）年化 12.4%、最大回撤 -16.2%
...
- **子样本稳健性（2010-01 起，16.6年）**：策略年化 13.1%（最大回撤 -16.2%）vs 买入持有年化 14.6%（最大回撤 -23.9%）——策略的超额收益高度依赖起点包含 2000 年泡沫顶；后半段策略主要贡献是降回撤而非增收益
- 各维度收紧月数：货币 40、财政 89、行政 87；锁激活 61 个月
