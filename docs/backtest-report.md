...
生成时间：2026-08-04 ｜ 数据源：FRED（利率 DFEDTAR+DFEDTARU 拼接、WALCL、MTSO133FMS+PCEPI平减(实际支出口径)、EPUTRADE、SAHMREALTIME）｜ 标普500：Tiingo SPY 总回报（adjClose，含股息复权）

## 方法论

- **重放粒度**：月末采样，319 个月（2000-01 起），逐月用与线上完全一致的阈值（`signal.config.js`）重算四维信号、萨姆锁/应对式调整锁与最终信号。未收官的当前日历月（月中重跑时只有半个月数据）从重放中剔除。
...
- 全面防守期月均收益：**-0.8%**（76 个月）｜减仓观望期：**1.4%**（182 个月）｜其余：**0.9%**（60 个月）
- **全面防守占比：24%**，减仓观望占比：57%（防守分级后，单维收紧不再全仓防守）
- 全期（26.5年）：买入持有 年化 8.4%、最大回撤 -50.8%；策略（仅defense离场，defense月计现金利息）年化 12.3%、最大回撤 -16.2%
- **敏感性（照档位建议执行）**："reduce=50%仓"策略（reduce 月 50% SPY + 50% 现金利息；defense 月全现金；其余满仓）：年化 8.1%、最大回撤 -10.6%——与上行"仅defense离场"口径并列，反映严格按档位执行的真实预期
- **不确定性区间（循环块自助法，2026-07-30 采纳）**：对逐月配对收益(策略,买持)做块长12个月、B=10000 的循环块自助——90% 区间：策略年化 [8.94%, 15.99%]、买持 [2.67%, 14.19%]、**年化差 [-0.4pp, +8.58pp]（优势为正概率约93%）**。区间下界跨零：与"优势集中于2000/2008两段大熊、无灾长牛期防守系统是净成本"的披露一致——引用点估计时应连同区间。复现：`node backtest/bootstrap-ci.mjs`（固定种子，读 backtest-raw.json 离线运行，重建口径与官方 summary 复现一致后才出区间；生成于 2026-08-04）。
- **子样本稳健性（2010-01 起，16.5年）**：策略年化 13.0%（最大回撤 -16.2%）vs 买入持有年化 14.5%（最大回撤 -23.9%）——策略的超额收益高度依赖起点包含 2000 年泡沫顶；后半段策略主要贡献是降回撤而非增收益
...
7. **执行时点敏感性（T+1 实测，2026-07-30）**：头条口径为信号日收盘成交（T+0，趋势门判定输入含成交那根收盘，理论上有轻微前视）。日度重放实测 T+1（次日收盘成交，即线上真实时序）：年化 12.781704858757248% vs T+0 12.08905155136437%（差 -0.7pp）、盯市回撤 T+1 -19.33875922421763% vs T+0 -19.33875922421764%（数字取自 daily-replay-raw.json 2026-08-04 快照，重跑 `node backtest/daily-replay.mjs` 刷新）；注意 2xQQQ 执行层结论不同（2022 年 T+0 -45.3% vs T+1 -36.5%，见 S5 手册），杠杆执行按 T+1 建模。
...
3. **精确率仍是主要代价**：71% 的防守片段未跟随大回撤，防守期月均收益 -0.8%（为负，防守有效规避了下跌；vs 非防守 0.9%）。若严格按信号空仓执行会错过部分上涨月份。
