...
| 今日API用量/配额 | 名下 key 今日开放API调用 / 生效配额档 | 生效档 = 最高 key 档，pro 需订阅有效否则按 free 计 |
| 近7天/近30天 | 全渠道（web+v1+mcp）调用次数 | `api_call_logs` 明细表（保留30天） |
| 调用总量 | 开放API历史底账 + 近30天web明细 | `api_usage`（400天）经 key 归属 + 明细 |
| 最后调用 | 全渠道最近一次调用时间 | |

操作：**启用/禁用**（禁用即时生效：bump `token_min_iat` 杀存量 JWT + 名下 key 视同禁用 +
登录返回 403）、**编辑订阅**（设到期时间即视为订阅用户；清空+取消勾选=退回免费）。
...
- 三渠道埋点：`/v1`（rateLimit 后挂 `req.usageMeta`）、`/mcp`（端点=工具名 `tool:<name>`）、
  `/api`（web，公开路由带 Bearer 头时埋点侧轻量 JWT 解码归属到人；登录/注册路由不记）。
- **写路径必须批量**：sql.js persist 是 O(库体积) 全库导出，明细在内存缓冲
  （`utils/usage-log.js`，上限5000条丢最旧），10分钟批量落库一次；管理端读接口先 flush 保证实时。
